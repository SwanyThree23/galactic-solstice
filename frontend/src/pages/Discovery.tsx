...
import { Heart, MessageCircle, Share2, DollarSign, Users, Eye, Radio, Grid, LayoutList } from 'lucide-react';
...
const Discovery: React.FC = () => {
    ...
    const demoStreams: StreamItem[] = [
        ...
        { id: '3', title: 'Elite 9-Guest Lobby Battle Royale', user: { id: 'u3', username: 'SeeWhyGamer', avatar: '' }, viewCount: 1245000, isLive: true, category: 'Gaming' },
    ...
    useEffect(() => {
        ...
        fetchStreams();
        const interval = setInterval(fetchStreams, 60000); // Poll every minute for live status/thumbnails
        return () => clearInterval(interval);
    }, []);

    const handleScroll = (e: React.WheelEvent) => {
        if (viewMode === 'grid' || streams.length === 0) return;
        if (e.deltaY > 0) {
    ...
    return (
        <div className="w-full h-screen bg-black overflow-hidden relative" onWheel={handleScroll}>
            {/* View Mode Toggle */}
            <div className="absolute top-6 left-6 z-[100] flex bg-black/40 backdrop-blur-xl p-1 rounded-2xl border border-white/5 shadow-2xl">
                <button
                    onClick={() => setViewMode('feed')}
                    title="Feed View"
                    className={`p-2 rounded-xl transition-all ${viewMode === 'feed' ? 'bg-red-600 text-white shadow-lg shadow-red-600/20' : 'text-gray-500 hover:text-white'}`}
                >
                    <LayoutList size={20} />
                </button>
                <button
                    ...
                    title="Grid View"
                    className={`p-2 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-red-600 text-white shadow-lg shadow-red-600/20' : 'text-gray-500 hover:text-white'}`}
                >
                    <Grid size={20} />
                </button>
            </div>
            ...
            {viewMode === 'grid' ? (
                <div className="w-full h-full overflow-y-auto p-12 pt-24 bg-[#050505] text-white">
                    <div className="max-w-7xl mx-auto">
                        <header className="mb-12">
                            <h2 className="text-5xl font-black brand tracking-tighter italic uppercase">Discover <span className="text-red-600">Live</span></h2>
                            <p className="text-gray-500 font-bold mt-4 uppercase tracking-[0.3em] text-[10px]">Real-time previews from the SeeWhy network</p>
                        </header>
                    ...
                    >
                        ...
                        <div className="absolute right-6 bottom-32 flex flex-col gap-8 z-20">
                            <div className="flex flex-col items-center gap-2">
                                ...
                                <button title="Follow User" className="bg-red-500 rounded-full w-5 h-5 -mt-4 text-xs flex items-center justify-center font-bold shadow-lg">+</button>
                            </div>
                        ...
                        <div className="absolute left-6 bottom-10 max-w-lg z-20">
                            ...
                            <div className="mt-4 flex gap-4 items-center">
                                ...
                                <span className="bg-white/5 px-3 py-1 rounded-full text-xs border border-white/5 font-bold">#seewhylive</span>
...
const EngagementButton = ({ icon, count, highlight = false }: any) => (
    <div className="flex flex-col items-center gap-1 cursor-pointer group text-white">
